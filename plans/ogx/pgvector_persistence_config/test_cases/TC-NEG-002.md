...
**Falsifiability**: reverting `expandPgvectorProvider` to `be51587` removes the emitted
reference, so step 3 fails and the pod does not start.
**Preconditions**:
...
4. Wait 300s for the server pod to reach `Ready`, then record its phase and
   `containerStatuses[?(@.name=="ogx")].restartCount`
...
- Step 4 shows `Running` with restart count `0`. The operator's obligation is the emitted
  reference in step 3. Whether the server resolves `kv_default` without a storage section is the
  server's behaviour, and a failure here is a finding against the server, not this fix
- Step 5 prints `0`
