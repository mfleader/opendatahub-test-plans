...
emits `persistence: {backend: kv_default, namespace: vector_io::pgvector}`, matching every other
vector_io provider in `config/samples/starter-config-configmap.yaml`.
...
- Milvus and Qdrant persistence, and the missing Milvus token key (tracked as follow-up issues)
- The pgvector `vector_index` shape mismatch with the OGX schema (tracked as a follow-up issue)
- `metadata_store` for praxis mode, which `applyPraxisVectorStoreMetadata` now supplies on `main`
- More than one declarative pgvector provider on one OGXServer. The custom resource allows up to
  100 and the fix hardcodes one namespace, so instances would share vector store records
...
| Base config ConfigMap, labelled `ogx.io/watch: "true"` | `version`, `distro_name`, `apis`, an `inference` provider, and for TC-E2E-003 and TC-E2E-004 the `starter-config-configmap.yaml` storage backends | Generation input, and the Objective #5 comparison fixture |
...
Eleven test cases live in [test_cases/](test_cases/), indexed in
...
| The operator sets no `SQLITE_STORE_DIR`, so a `kv_default` `db_path` copied from `starter-config-configmap.yaml` resolves outside the PVC mounted at `/.ogx`, to a path that may be read-only | Medium | Medium | Record the resolved path if startup fails and file the unset `SQLITE_STORE_DIR` separately. The missing setting is a base-config defect (Objective: #2) |
